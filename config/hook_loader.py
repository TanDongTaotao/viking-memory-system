...
class HookConfig:
    ...
    def __init__(self, name: str, script: str, timeout: int = DEFAULT_TIMEOUT,
                 ...
                 args: Optional[List[str]] = None, background: bool = False):
        self.name = name
        self.script = script
...
class HooksManager:
    ...
    def _load_config(self) -> None:
        ...
        try:
            with open(self.config_path, "r", encoding="utf-8") as f:
                ...
                # 支持两种格式：
                # 1. 新格式（列表）: hooks: [{when: {agent: xxx}, on_session_start: [...]}]
                # 2. 旧格式（字典）: hooks: {on_session_start: [...], on_session_end: [...]}
                
                if isinstance(hooks_data, list):
                    # 新格式：遍历每个 hook 配置块
                    for hook_block in hooks_data:
                        when_config = hook_block.get("when", {})
                        target_agent = when_config.get("agent", "")
                        
                        # 处理 on_session_start
                        for hook_data in hook_block.get("on_session_start", []):
                            # 为钩子添加目标 agent 信息
                            hook_data["_target_agent"] = target_agent
                            name = hook_data.get("name", "unnamed")
                            
                            if "on_session_start" not in self.hooks:
                                self.hooks["on_session_start"] = []
                            self.hooks["on_session_start"].append(HookConfig.from_dict(name, hook_data))
                        
                        # 处理 on_session_end
                        for hook_data in hook_block.get("on_session_end", []):
                            hook_data["_target_agent"] = target_agent
                            name = hook_data.get("name", "unnamed")
                            
                            if "on_session_end" not in self.hooks:
                                self.hooks["on_session_end"] = []
                            self.hooks["on_session_end"].append(HookConfig.from_dict(name, hook_data))
    ...
    def _execute_hook(self, hook: HookConfig, agent_name: str = "",
        ...
        if hook.background:
            ...
            logger.info(f"[后台] 启动钩子 [{hook.name}]: {' '.join(full_cmd)}")
            ...
            try:
                ...
                logger.error(f"后台启动钩子 '{hook.name}' 失败: {e}")
                return None
